...
methods {
    ...
    function _._updateReward(address account) internal => updateRewardCVL(account) expect void;
    function _.earned(address account) external => earnedCVL(account) expect uint256;
...
ghost mathint updateRewardCalled;

function updateRewardCVL(address account) {
    updateRewardCalled = updateRewardCalled + 1;
}

function earnedCVL(address account) returns uint256 {
    return currentContract.rewards[account];
}

// manual linking, use in your rules and invariants
function setup() {
    require getExtraRewarder(0) == extraRewarder;
    require getExtraRewarder(1) == extraRewarder;
}
